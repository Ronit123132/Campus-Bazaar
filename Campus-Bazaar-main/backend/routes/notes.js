...
router.post('/purchase/:id', authMiddleware, async (req, res) => {
  try {
    const note = await Note.findById(req.params.id);
    if (!note) {
      console.error('Purchase error: Note not found', req.params.id);
      return res.status(404).json({ message: 'Note not found' });
    }

    if (!note.isForSale) {
      console.error('Purchase error: Note is not for sale', note._id);
      return res.status(400).json({ message: 'This note is not for sale' });
    }

    if (!note.uploader) {
      console.error('Purchase error: Note uploader missing', note._id);
      return res.status(400).json({ message: 'Note uploader missing' });
    }

    if (note.uploader.toString() === req.user.userId) {
      console.error('Purchase error: User tried to purchase own note', note._id, req.user.userId);
      return res.status(400).json({ message: 'You cannot purchase your own note' });
    }

    const buyer = await User.findById(req.user.userId);
    if (!buyer) {
      console.error('Purchase error: Buyer not found', req.user.userId);
      return res.status(400).json({ message: 'Buyer not found' });
    }
    const seller = await User.findById(note.uploader);
    if (!seller) {
      console.error('Purchase error: Seller not found', note.uploader);
      return res.status(400).json({ message: 'Seller not found' });
    }

    if (typeof note.price !== 'number' || isNaN(note.price)) {
      console.error('Purchase error: Note price invalid', note.price);
      return res.status(400).json({ message: 'Note price invalid' });
    }

    // Check if already purchased
    const alreadyPurchased = Array.isArray(note.purchasedBy) && note.purchasedBy.some(
      purchase => purchase.user && purchase.user.toString() === req.user.userId
    );
    if (alreadyPurchased) {
      console.error('Purchase error: Already purchased', note._id, req.user.userId);
    ...
    res.status(500).json({ message: 'Error processing purchase', error: error.message, stack: error.stack });
  }
});
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    console.log('Note uploader:', note.uploader.toString(), 'Requesting user:', req.user.userId);
    if (note.uploader.toString() !== req.user.userId) {
