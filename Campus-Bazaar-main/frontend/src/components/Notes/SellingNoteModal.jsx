...
export default function SellingNoteModal({ isOpen, onClose, note, onPurchase, user }) {
  ...
  if (user && Array.isArray(user.notesPurchased)) {
    isPurchased = user.notesPurchased.some(
      (purchase) => {
        if (typeof purchase === 'object' && purchase.note) {
          return purchase.note === note._id;
        }
        return purchase === note._id;
      }
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 max-w-lg w-full relative animate-fade-in flex flex-col items-center">
        ...
        {!isOwner ? (
          isPurchased ? (
            <a
              href={note.fileUrl?.startsWith('http') ? note.fileUrl : `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}${note.fileUrl}`}
              download
              className="px-5 py-2 rounded-lg bg-green-600 text-white font-bold shadow hover:bg-green-700 transition w-full text-center"
            >
              Download
            </a>
          ) : showPayment ? (
